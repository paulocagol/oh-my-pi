...
/** Captured before any test chdirs away from it. */
...
describe("omp grep internal-URL branch", () => {
	...
	afterEach(async () => {
		...
		// `setProjectDir` chdirs the process, so the temp root has to be handed
		// back before it is deleted: every later test in this run would inherit a
		// cwd that no longer exists, and anything resolving a relative path or
		// spawning a child would fail far from here.
